...
public class ChordLookup {
	...
	*/
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		NodeInterface upper = Util.getProcessStub(node.getSuccessor().getNodeName(), node.getSuccessor().getPort());
		return (Util.computeLogic(key, node.getNodeID().add(BigInteger.valueOf(1)), upper.getNodeID())) ? upper : findHighestPredecessor(key).findSuccessor(key);				
	}
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		List<NodeInterface> list = node.getFingerTable();
		for (int i = list.size() - 1; i >= 0; i--) {
			NodeInterface finger = Util.getProcessStub(list.get(i).getNodeName(), list.get(i).getPort());
			if (Util.computeLogic(finger.getNodeID(), node.getNodeID().add(new BigInteger("1")), key.subtract(new BigInteger("1")))) return finger;
		}
