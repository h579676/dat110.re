 ...
 */
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	for (BigInteger file : replicafiles) {
    		NodeInterface successor = chordnode.findSuccessor(file);
    		successor.addKey(file);	
    		successor.saveFileContent(filename, file, bytesOfFile, counter == new Random().nextInt(Util.numReplicas - 1) ? true : false); 
  			counter++;
   		}
		return counter;
    }
  ...
  */
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (BigInteger file : replicafiles) {
    		NodeInterface successor = chordnode.findSuccessor(file);
    		succinfo.add(successor.getFilesMetadata(file));
		}
	...
	public NodeInterface findPrimaryOfItem() {
		for(Message message : activeNodesforFile) {
			if (message.isPrimaryServer()) Util.getProcessStub(message.getNodeIP(), message.getPort());
		} 
		return null;	
	}
